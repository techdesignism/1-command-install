...
7.3
```
apt update && apt -y install software-properties-common && add-apt-repository -y ppa:ondrej/php  && add-apt-repository -y ppa:tomvlk/php-archive && apt update && apt -y install php7.3 && apt -y install curl unzip && apt install -y php7.3-{bcmath,bz2,intl,gd,mbstring,mysql,zip,common,curl} && curl -sS https://getcomposer.org/installer -o composer-setup.php && php composer-setup.php --install-dir=/usr/local/bin --filename=composer && composer self-update && apt -y install php7.3-mbstring  php7.3-xml php7.3-sqlite3 && php -v && composer -V
```
